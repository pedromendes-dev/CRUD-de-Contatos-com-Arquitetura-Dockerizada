import { LogoProps } from "../../utils/types/ContatoLogoTypes";
...
export function Logo({
  ...
  if (variant === "badge") {
    return (
      <div
        className={`inline-flex items-center gap-2.5 px-3 py-2 ${className}`}
        style={{ backgroundColor: "#101828", borderRadius: 10 }}
      >
        <svg
          width={size}
          height={size}
        ...
        {showText && (
          <div className="flex flex-col gap-0.5">
            <span className="font-bold text-lg leading-none text-white">SmartReg</span>
            <span className="text-xs leading-none" style={{ color: "#6a7282" }}>
              Sistema Inteligente
            </span>
          </div>
        )}
  ...
  const colorClass =
    variant === "white" ? "text-white" : variant === "dark" ? "text-gray-900" : "[color:#155dfc]";
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      ...
      {showText && (
        <div className="flex flex-col leading-tight" style={{ fontSize: textSize }}>
